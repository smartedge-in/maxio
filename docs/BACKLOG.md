...
| ~~P3-25~~ | ~~Optional edge LB (`maxio-edge` / Pingora)~~ | ops | L | **Dropped** — Ingress (P3-19), nginx/Caddy (operations.md), and K8s Service cover client-facing LB; VIP via keepalived (P3-18). A first-party Pingora edge duplicates those paths with high maintenance cost (Linux-only, MSRV, extra binary) for little differentiation. Embedded Pingora in `maxio-server` also rejected. See `docs/plans/2026-06-30-pingora-edge-lb.md`. | — |
