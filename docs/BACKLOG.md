...
| P3-13 | Asymmetric scale-out (server vs storage) | storage | XL | P3-04 split `maxio-storage` / `maxio-server` at compile time only; runtime is still one binary with colocated `data_dir`. Operators cannot scale API/console tier independently of storage (e.g. 4× server pods, 1× storage node). Requires `StorageBackend` trait (P3-10) plus remote or routed storage access so `maxio-server` is stateless at the edge. | `maxio-server` runs with remote storage backend (no local `data_dir` required); `maxio-storage` deployable as storage-only role; K8s/deployment docs with independent replica counts and HPA per tier; integration test: ≥2 server instances share one storage backend with consistent reads after write; single-binary colocated mode remains default. |
