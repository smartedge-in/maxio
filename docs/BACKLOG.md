...
| P3-25 | Optional edge LB (`maxio-edge` / Pingora) | ops | L | Optional **separate** HTTP edge using [Pingora](https://github.com/cloudflare/pingora) (Apache-2.0). **Rejected:** rebuilding `maxio-server` on Pingora for VIP/LB — VIP is network/K8s layer; routing is Server/Storage Raft (P3-15/14). See `docs/plans/2026-06-30-pingora-edge-lb.md`. | Optional `maxio-edge` binary only (not embedded in server); upstream pools for server/UI; VIP via keepalived or K8s Service documented in P3-18/19; no Axum→Pingora server rewrite. |
