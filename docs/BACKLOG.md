...
| P3-18 | Bare metal deployment pack | ops | M | MaxIO must support native Linux bare-metal/VM installs, not only containers. Today `docs/operations.md` lacks systemd units, install/upgrade steps, and multi-host layout guidance. See `docs/plans/2026-06-29-deployment-targets.md`. | `deploy/systemd/maxio.service`; bare-metal section in `docs/operations.md` (binary install, `data_dir`, firewall, TLS proxy, upgrade); optional distributed multi-host layout doc stub for P3-13; smoke test via `maxio healthcheck` in CI or documented manual checklist. |
...
| P3-25 | Native edge LB (`maxio-edge` / Pingora) | ops | L | Optional **HTTP edge** load balancer using [Cloudflare Pingora](https://github.com/cloudflare/pingora) (Apache-2.0) in front of **maxio-server** and **maxio-ui** — not storage tier. Storage uses Raft + `StorageBackend` (P3-14), not HTTP round-robin. See `docs/plans/2026-06-30-pingora-edge-lb.md`. | `crates/maxio-edge` (or binary) with upstream pools, `/readyz` health checks, TLS termination docs; integration test PUT/GET via edge; complements P3-19 Ingress / nginx, not a replacement for Server/Storage Raft. |
