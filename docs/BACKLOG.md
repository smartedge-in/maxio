...
| 4 | P1-17 | Storage tier Raft | storage | XL | **In progress** — metadata consensus: buckets, object index, multipart, bucket settings, keyring epoch. Object bytes local per node. *Was P3-14.* | 3-node bootstrap/join; writes via leader; follower failover integration test; metrics `raft_storage_leader`, `raft_storage_commit_lag`. |
